...
class TestBackup:
    ...
    @pytest.mark.asyncio
    async def test_backup_info(self, client, auth_headers):
        """Test getting backup info"""
        if not auth_headers:
            pytest.skip("No auth token available")
        
        response = await client.get("/api/backup/info", headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_backup_export(self, client, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_backup_info_unauthenticated(self, client):
        """Test backup info without authentication"""
        response = await client.get("/api/backup/info")
...
class TestClassificacao:
    ...
    @pytest.mark.asyncio
    async def test_get_classificacao_codigos(self, client, auth_headers):
        """Test getting classification codes"""
        response = await client.get("/api/classificacao/codigos")
        # This endpoint might be public or require auth
        if response.status_code == 401:
            response = await client.get("/api/classificacao/codigos", headers=auth_headers)
