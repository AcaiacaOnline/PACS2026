...
const PortalPublico = () => {
  ...
  const renderPacsGeralObras = () => (
    <div className="space-y-4">
      ...
      <div className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-xl shadow overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {pacsGeralObras
              .filter(p => 
                p.nome_secretaria?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                p.secretario?.toLowerCase().includes(searchTerm.toLowerCase())
              )
              .map((pac) => (
              <tr key={pac.pac_obras_id} className="border-t hover:bg-amber-50/50">
                <td className="px-4 py-3 text-sm font-medium">{pac.nome_secretaria}</td>
                <td className="px-4 py-3 text-sm">{pac.secretario}</td>
                <td className="px-4 py-3 text-sm">{pac.fiscal_contrato || '-'}</td>
                <td className="px-4 py-3 text-sm">{pac.ano}</td>
                <td className="px-4 py-3 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <button
                      onClick={() => fetchItemDetails('pac-geral-obras', pac.pac_obras_id)}
                      className="text-blue-600 hover:text-blue-800 p-1"
                      title="Ver Itens"
                    >
                      <Eye size={18} />
                    ...
                    <button
                      onClick={() => handleExportPDF('pac-geral-obras', pac.pac_obras_id)}
                      className="text-red-600 hover:text-red-800 p-1"
                      title="Exportar PDF"
                      disabled={exporting}
                    >
            ...
            {pacsGeralObras.length === 0 && (
              <tr>
                <td colSpan="5" className="px-4 py-8 text-center text-gray-500">
                  Nenhum PAC de Obras e Serviços cadastrado.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {selectedItem?.type === 'pac-geral-obras' && itemDetails.length > 0 && (
        <div className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-xl shadow overflow-hidden mt-4">
          <div className="bg-amber-50 px-4 py-3 border-b flex justify-between items-center">
            <h3 className="font-semibold text-gray-800">Itens de Obras e Serviços ({itemDetails.length})</h3>
            <div className="flex items-center gap-2">
              <button
                onClick={() => handleExportPDF('pac-geral-obras', selectedItem.id, 'landscape')}
                className="flex items-center gap-1 text-sm bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700"
                disabled={exporting}
              >
                <Download size={14} />
              ...
              <button
                onClick={() => handleExportPDF('pac-geral-obras', selectedItem.id, 'portrait')}
                className="flex items-center gap-1 text-sm bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700"
                disabled={exporting}
              >
                <Download size={14} />
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-left font-medium">Descrição</th>
                  <th className="px-3 py-2 text-left font-medium">Classificação</th>
                  <th className="px-3 py-2 text-center font-medium">Unidade</th>
                  <th className="px-3 py-2 text-center font-medium">Qtd Total</th>
                  <th className="px-3 py-2 text-right font-medium">Valor Unit.</th>
                  <th className="px-3 py-2 text-right font-medium">Valor Total</th>
                </tr>
                  ...
                  <tr key={item.item_id} className="border-t">
                    ...
                    <td className="px-3 py-2">
                      <div className="bg-amber-50 px-2 py-1 rounded">
                        <span className="font-bold text-amber-800">{item.codigo_classificacao}</span>
                        {item.subitem_classificacao && (
                          <span className="block text-amber-600 text-xs">{item.subitem_classificacao}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-3 py-2 text-center">{item.unidade}</td>
              ...
              <tfoot className="bg-amber-100">
                <tr>
                  ...
                  <td className="px-3 py-2 text-right font-bold">
                    {formatCurrency(itemDetails.reduce((sum, item) => sum + (item.valorTotal || 0), 0))}
                  </td>
                </tr>
              </tfoot>
            </table>
